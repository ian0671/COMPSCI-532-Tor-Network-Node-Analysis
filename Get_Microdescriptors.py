...
# app.register_functions(CollecTor_Get_Microdescriptors)
#
...
CollecTor_Get_Microdescriptors = func.Blueprint()

# ---------------- PRODUCER ----------------
@CollecTor_Get_Microdescriptors.timer_trigger(
    schedule="0 2 0 * * *",
    arg_name="myTimer",
    run_on_startup=True,
    use_monitor=False
)
def Get_Microdescriptors(myTimer: func.TimerRequest) -> None:
    logging.info("Get_Microdescriptors function started.")

    connection_str = os.getenv("EVENTHUB_CONNECTION")
    eventhub_name = os.getenv("Get_Microdescriptors_EH_NAME")
    ...
    try:
        descriptors = fetch_microdescriptors_since(hours_back=24, limit=None)
        logging.info("Collected %d microdescriptors from CollecTor", len(descriptors))
        ...
        with producer:
            ...
            for idx, desc in enumerate(descriptors):
                logging.debug("🔍 Microdescriptor at index %d:\n%s",
                              idx, json.dumps(desc, indent=2, default=str))

                if not isinstance(desc, dict):
                    logging.warning("⛔ Skipping non-dict microdescriptor at index %d: %s",
                                    idx, repr(desc))
                    continue

                tagged = {
                    "source": "Get_Microdescriptors",
                ...
                if not tagged_json:
                    logging.warning("⛔ Skipping empty microdescriptor payload at index %d", idx)
                    continue

                logging.debug("📤 [%d] Sending microdescriptor with fingerprint: %s\n%s",
                              idx,
        ...
        logging.info("✅ Sent %d microdescriptors to Event Hub", sent_count)

        # 🔽 Immediately consume after producing
        consume_events(connection_str, eventhub_name, expected_count=sent_count, timeout=30)

    except Exception:
        logging.exception("❌ Scheduled CollecTor: get_microdescriptors send failed")
        raise
    finally:
...
def consume_events(connection_str: str, eventhub_name: str,
    ...
    client = EventHubConsumerClient.from_connection_string(
        conn_str=connection_str,
        consumer_group="get_microdescriptors_consumer_group",  # ensure this exists
        eventhub_name=eventhub_name
    )
...
def fetch_microdescriptors_since(hours_back: int = 24,
                                 limit: Optional[int] = None) -> List[Dict[str, Any]]:
    """
    Fetch microdescriptors published since (now - hours_back) and normalize them.
    Uses aware UTC in application logic, converts to naive UTC for Stem call.
    """
    from stem.descriptor.collector import get_microdescriptors

    aware_start = get_aware_utc_start(hours_back)
    naive_start = aware_to_naive_utc(aware_start)

    raw_iter = get_microdescriptors(start=naive_start)

    results = list(raw_iter)
    if limit is not None:
        results = results[:limit]

    return [_normalize_microdescriptors(r) for r in results]