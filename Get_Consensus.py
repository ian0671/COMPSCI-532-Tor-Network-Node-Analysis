...
CollecTor_Get_Consensus = func.Blueprint()

# ---------------- PRODUCER ----------------
@CollecTor_Get_Consensus.timer_trigger(
    schedule="0 6 0 * * *",
    arg_name="myTimer",
    run_on_startup=True,
    use_monitor=False
)
def Get_Consensus(myTimer: func.TimerRequest) -> None:
    logging.info("Get_Consensus function started.")

    connection_str = os.getenv("EVENTHUB_CONNECTION")
    eventhub_name = os.getenv("Get_Consensus_EH_NAME")
    ...
    try:
        descriptors = fetch_consensus_since(hours_back=24, limit=None)
        logging.info("Collected %d consensus from CollecTor", len(descriptors))
        ...
        with producer:
            ...
            for idx, desc in enumerate(descriptors):
                logging.debug("🔍 Consensus at index %d:\n%s",
                              idx, json.dumps(desc, indent=2, default=str))

                if not isinstance(desc, dict):
                    logging.warning("⛔ Skipping non-dict consensus at index %d: %s",
                                    idx, repr(desc))
                    continue

                tagged = {
                    "source": "Get_Consensus",
                    "type": "consensus",
                ...
                logging.debug("📤 [%d] Sending consensus with fingerprint: %s\n%s",
                              idx,
        ...
        logging.info("✅ Sent %d consensus to Event Hub", sent_count)

        # 🔽 Immediately consume after producing
        consume_events(connection_str, eventhub_name, expected_count=sent_count, timeout=30)

    except Exception:
        logging.exception("❌ Scheduled CollecTor: get_consensus send failed")
        raise
    finally:
...
def consume_events(connection_str: str, eventhub_name: str,
    ...
    client = EventHubConsumerClient.from_connection_string(
        conn_str=connection_str,
        consumer_group="get_consensus_consumer_group",
        eventhub_name=eventhub_name
    )
...
# ---------------- HELPERS ----------------
def _normalize_consensus(desc: Any) -> Dict[str, Any]:
    return {
...
def fetch_consensus_since(hours_back: int = 24,
    ...
    """
    from stem.descriptor.collector import get_consensus

    aware_start = get_aware_utc_start(hours_back)
    naive_start = aware_to_naive_utc(aware_start)

    raw_iter = get_consensus(start=naive_start)

    results = list(raw_iter)
    if limit is not None:
        results = results[:limit]

    return [_normalize_consensus(r) for r in results]