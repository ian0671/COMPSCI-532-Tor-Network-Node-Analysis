...
import pandas as pd
import pyarrow  # noqa: F401 imported for parquet engine availability
import json
...
def sanitize_for_parquet(df: pd.DataFrame) -> pd.DataFrame:
    """Ensure all columns are primitive (string/number/bool) types.

    Azure ML / Stream ingestion can fail on complex or mixed object columns
    producing Parquet logical types not supported by preview tooling.
    This converts any remaining complex python objects to JSON strings and
    forces pure object columns to string dtype.
    """
    for col in df.columns:
        if df[col].dtype == 'O':
            series = df[col].apply(_serialize_complex)
            # If after serialization we still have mixed types, cast to string
...
def main(blob: func.InputStream):
    ...
    # Extract container name from blob.uri (format: https://<account>.blob.core.windows.net/<container>/<blob>)
    ...
    with tempfile.TemporaryDirectory() as tmpdir:
        ...
        # Write with pyarrow to ensure consistent logical types
